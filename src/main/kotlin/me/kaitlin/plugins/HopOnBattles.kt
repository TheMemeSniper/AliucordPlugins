...
class HopOnBloonsWhen : Plugin() {
    ...
    override fun start(ctx: Context) {
        ...
        observable = StoreStream.getGatewaySocket().messageCreate.onBackpressureBuffer().subscribe {
            ...
            if (content.contains("hop on battles") || content.contains("get on battles") ||
                content.contains("com.ninjakiwi.bloonstdbattles2")
            ) {
                hopOnBloons()
            }
        }
    ...
    private fun hopOnBloons() {
        try {
            val bloons =
                Utils.appContext.packageManager.getLaunchIntentForPackage("com.ninjakiwi.bloonstdbattles2")
